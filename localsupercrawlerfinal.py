...
def main():
    st.set_page_config(page_title="Enhanced Web Crawler", page_icon="ð���", layout="wide")
    ...
    with col1:
        start_btn = st.button("ð��� Start Crawling", help="Begin crawling")
    with col2:
        stop_btn = st.button("â�� Stop & Reset", help="Stop and reset")
    ...
    if st.session_state.running and st.session_state.crawler:
        ...
        if st.session_state.results:
            with results_container.container():
                ...
                with col1:
                    if st.button("ð��¾ Save Results & Stop", key="save_results"):
                ...
                with col2:
                    if st.button("ð��� Continue to Next Category", key="next_category") and crawler.crawl_mode != "Complete":
                        crawler.pages_crawled = 0
                with col3:
                    if st.button("â�© Continue Current Category", key="current_category") and crawler.crawl_mode != "Complete":
                        crawler.pages_crawled = 0
                st.markdown('</div>', unsafe_allow_html=True)
        ...
        if not st.session_state.running and st.session_state.results and not crawler.user_stopped:
            csv_data = generate_csv(crawler.results)
            st.download_button(
                label="ð��¥ Download Final Results",
                data=csv_data,
                file_name=f"crawl_report_{datetime.datetime.now().strftime('%Y%m%d_%H%M%S')}.csv",
                mime="text/csv"
            )
